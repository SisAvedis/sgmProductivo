...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	$res = $DB->prinUltimoEnviadov41a($fecha,$cliente,$detallepor,$propiedad,$estado,$tproc,$producto);
	...
	if($row_cnt == 0){
		...
		while($row = mysqli_fetch_array($res)){
			...
			$audit_arr[$it][5] = date_format(date_create($row['fecha']),"d-m-Y");
			$audit_arr[$it][6] = $row['remito'];
			$it++;
		}
		...
		$header1 = "<table class='table table-bordered' id='tablaUno'><thead><tr><th class='text-left' colspan=7><label name='lbcantidadE' id='lbcantidadE'><pre class='lx-pre'>Total de Registros: ".$row_cnt." | Registros Insertados Manualmente: 0"."</pre></label></th>";
		
		$header1 = $header1."<th class='text-center' colspan=3><label><pre class='lx-pre'>Insertar Registros</pre></label></th><th class='text-center'><pre class='lx-pre'><button class='btn btn-success' id='btnadd' onclick='jq.fn.insertaRegistroAuditoria()'>+</button></pre></th>";
		...
		for($tre=0;$tre<$row_cnt;$tre++){
			...
			$header1 = $header1."<td><pre class='lx-pre'><input type='text' name='".$sFEC."' class='form-control input-sm' id='".$sFEC."' maxlength=10  autocomplete='off' value=".$audit_arr[$tre][5]." disabled/></pre><input type='hidden' value=".$audit_arr[$tre][5]." name='".$sFEH."' id='".$sFEH."'/></td>";
			$header1 = $header1."<td><pre class='lx-pre'><input type='text' name='".$sREM."' class='form-control input-sm' id='".$sREM."' maxlength=6  autocomplete='off' value=".$audit_arr[$tre][6]." disabled/></pre><input type='hidden' value=".$audit_arr[$tre][6]." name='".$sREH."' id='".$sREH."'/></td>";			
			...
			$header1 = $header1."<td><pre class='lx-pre'><input type='text' name='".$sLOT."' class='form-control input-sm lote' id='".$sLOT."' maxlength=5  autocomplete='off' /></pre><input type='hidden' name='".$sLOH."' id='".$sLOH."' value=0 /></td>";
