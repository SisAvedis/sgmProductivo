...
<?php try{ ?>
<body>
	<form method="post" action="remitoAltaSI-v9.php" id="frm" >
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if($propiedad && $tipoenv && validaSerie($nremito)){
				...
				$tipoRemito = 'RSUBI';
				$res = $DB->prInsertarCSVSIv8($fecha,$nremito,$clienteid[1],$productoid[0],$productoid[1],$seriesCSV,$prop,$tipoenvase,$clienteid[0],$tipoRemito,$idusuario);
...
<script type="text/javascript">
	...
	function chkRemitoExiste(sRemito, sTipo, callbackFn){
		if(sRemito.trim() !== ''){
			jq.ajax({
				url:"./ajaxConsultaRemitoSI-v9.php",
				dataType: "json",
	...
	function chkRemitoExisteUno(sRemito, callbackFn){
		if(sRemito.trim() !== ''){
			jq.ajax({
				url:"./ajaxConsultaRemito-v12.php",
				dataType: "json",
	...
	function getNumeroRemito(callbackFn){
		jq.ajax({
			url:"./ajaxGetNumeroRemito-v3.php",
			dataType: "json",
			type:"POST",
			success:function(response){
				callbackFn(response);
