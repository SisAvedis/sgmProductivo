	...
	if($row_cnt == 0){
		...
		while($row = mysqli_fetch_array($res)){
			if ($it == 0){
				...
				//$audit_arr[$it-1][6] = $row['remito'];
				$audit_arr[$it-1][7] = $row['auditado'];
				$audit_arr[$it-1][8] = $row['tipo'];
				$it++;	
			}
			...
			*/
			...
			foreach($select_arr2 as $sel2){
				...
				if ($id_xl[0] == $audit_arr[$tre][4]){
					...
					$header4 = $header4.$id_xl[0].";".$audit_arr[$tre][8].";";
					//$select_arr[$iRow2] = "<option value=".$id_xl[0]."|".$id_xl[1]."|".$iRow2." selected>".$id_xl[0]." - ".$id_xl[1]."</option>";
				}else{
			...
			$header1 = $header1."<td><pre class='lx-pre'><input type='text' name='".$sLOT."' class='form-control input-sm lote' id='".$sLOT."' maxlength=6  autocomplete='off' /></pre><input type='hidden' name='".$sLOH."' id='".$sLOH."' value='0' /></td>";
			...
			if ($audit_arr[$tre][7] == 1){
				$header1 = $header1."checked";
			}
			$header1 = $header1." /></pre><input type='hidden' name='".$sCHKAH."' id='".$sCHKAH."' value='".$audit_arr[$tre][7]."' /></td>";
			$header1 = $header1."<td class='text-center'><label id='".$sTYA."'><pre class='lx-pre'>".$audit_arr[$tre][8]."</pre></label><input type='hidden' name='".$sTYH."' value='".$audit_arr[$tre][8]."' id='".$sTYH."' /></td>";
