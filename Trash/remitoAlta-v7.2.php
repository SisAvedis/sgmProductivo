...
<?php try{ ?>
<body>
	<form method="post" action="remitoAlta-v7.2.php" id="frm" >
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if($otipoProducto && $otipoProducto == 'LG' && validaSerie($nremito)){
				...
				$res = $DB->prInsertarCSVv66($fecha,$nremito,$clienteid[1],$seriesCSV,$prop,$tipoenvase,$clienteid[0],$tipoRemito,$motivo,$idusuario);
...
<script type="text/javascript">
	...
	function chkRemitoExiste(sRemito, sTipo, nradioVal, callbackFn){
		...
		if(sRemito.trim() !== ''){
			jq.ajax({
				//09-09-22
				url:"./ajaxConsultaRemito-v7.2.php",
				//09-09-22 - Fin
	...
	jq(document).ready(function(){
		...
		console.log("Adentro despues de document ready...");
		...
		jq.fn.chkData = function(){
			...
			//console.log('envEn -> '+envEn+' envStr -> '+envStr+' ecapStr -> '+ecapStr+' ecapStr0 -> '+ecapStr0)
			...
			for(j = 0 ;j < 24 ;j++){
				if(jq('#'+aSerieCapFull[2][j]).prop('checked') == true){
					if(jq('#'+aSerieCapFull[4][j]).text() == '' || jq('#'+aSerieCapFull[4][j]).text() == ' Cap: 0'){
						...
						console.log(aSerieCapFull[6][j]+' || '+sCapArr[0]);
					}
				}
			}
