...
<?php try{ ?>
<body>
	<form method="post" action="remitosBM-v2.php" id="frm" >
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div id="tit" class="col-sm-8"><h4>Remito BM(v2)</h4></div>
                </div>
            </div>
			...
			if($otipoProducto && $otipoProducto == 'LG' && validaSerie($nremito)){
				...
				$res = $DB->prInsertarCSVv6($fecha,$nremito,$clienteid[1],$seriesCSV,$prop,$tipoenvase,$clienteid[0]);
				...
				<?php
						...
						$res = $DB->prInsertarGRv1($fecha,$nremito,$clienteid[1],$estado,$serie,$volumen,$prop,$tipoenvase,$clienteid[0],$partida,$certificado,$tdestino[0]);
						$row_cnt = mysqli_num_rows($res);
						if ($row_cnt == 0){
							$message= "No se insertaron movimientos";
							$class="alert alert-info";
...
<script type="text/javascript">
	...
	function chkRemitoExiste(sRemito, callbackFn){
		//console.log("Valor de nradioVal -> "+nradioVal);
		if(sRemito.trim() !== ''){
			jq.ajax({
				url:"./ajaxConsultaRemitoBM-v2.php",
				dataType: "json",
	...
	jq(document).ready(function(){
		...
		jq.fn.inputradioProducto = function(){
			...
			chkRemitoExiste(sRemito, function(data) { // data = the JSON retrieved
				var len = data.length;
				...
				for(var i=0; i<len; i++){
					...
					if(bRemitoIngresado == true){
						...
						var sMSG16 = data[i].header16;
						var sLbl16 = "<label id='die'><pre class='lx-pre'></pre></label></br>";
						jq("#lblExisteRemito").append(sLbl16);
						jq("#die pre").attr('id', 'msg16');
						jq('#msg16').css("background-color","#E8F7AC");
						jq('#msg16').css("color","#000333");
						jq("#msg16").text(sMSG16);
						
						
						var header17 = data[i].header17;
						
						
						jq("#lblExisteRemito").html(sMSG16);
						jq("#lblExisteRemito").html(header17);
		...
		jq("#nremito").focusout('input', function(){
			console.log("Entrando en focusout de input..."),
			sRemito = jq(this).val();
			jq.fn.inputradioProducto();
			//var envOpcion = jq(".env:checked").val();
