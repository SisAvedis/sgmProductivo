...
<?php try{ ?>
<body>
	<form method="post" action="remitoBM-v1.2.php" id="frm">
    <div class="container">
		<!-- HEADER (start) -->
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			if($tipo <> "Granel"){
				...
				if ($bmOpcion == "B"){
					...
					$res = $DB->prBMRemitoLGv12($fecha,$fecha,$nremito,$nremito,$nid_xl,$nid_xl,$pr,$pr,$tipo,$bmOpcion,$idusuario);
					...
					$res = $DB->prBMRemitoLGv12($fecha,$fechaM,$nremito,$nremitomodi,$nid_xl,$clienteid[0],$pr,$prM,$tipo,$bmOpcion,$idusuario);
					$row_cnt = mysqli_num_rows($res);
				}
				?>
			...
			<?php		
				...
				if ($bmOpcion == "B"){
					...
					$res = $DB->prBMRemitoGRv1($fecha,$fecha,$nremito,$nremito,$nid_xl,$nid_xl,$vopacetaid[0],$vopacetaid[0],
					...
					$res = $DB->prBMRemitoGRv1($fecha,$fechaM,$nremito,$nremitomodi,$nid_xl,$clienteid[0],$vopacetaid[0],$vol,
