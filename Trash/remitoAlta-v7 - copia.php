...
<?php try{ ?>
<body>
	<form method="post" action="remitoAlta-v7.php" id="frm" >
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if($otipoProducto && $otipoProducto == 'LG' && validaSerie($nremito)){
				...
				$res = $DB->prInsertarCSVv5($fecha,$nremito,$clienteid[1],$seriesCSV,$prop,$tipoenvase,$clienteid[0]);
...
<script type="text/javascript">
	...
	function setSerie(sSerie, sProp, sTipo, callbackFn) {
		jq.ajax({
			url:"./ajaxConsultaSerie.php",
			dataType: "json",
			data: {"sSerie": sSerie, "sProp": sProp, "sTipo": sTipo },
			type:"POST",
			success:function(response){
				callbackFn(response);
			}
	...
	jq(document).ready(function(){
		...
		jq("#lblRemito6").on('focusout','.form-control', function(){
			...
			if(sSerie !== ''){
				setSerie(sSerie, sProp, sTipo, function(data){// data = the JSON retrieved
				var len = data.length;
				for(var i=0; i<len; i++){
		...
		jq("#lblRemito4").on('focusout','#part', function(){
			bcertVal = jq.fn.chkPartida(jq(this).val());
			if(bcertVal == true){
				jq(this).css("background-color","#92FF8A");
				jq(this).css("color","#000333");
			}else{
		...
		jq("#lblRemito5").on('focusout','#cert', function(){
			bpartVal = jq.fn.chkCertificado(jq(this).val());
			if(jq(this).val() !== ""){
				if(bpartVal == true){
					jq(this).css("background-color","#92FF8A");
					jq(this).css("color","#000333");
