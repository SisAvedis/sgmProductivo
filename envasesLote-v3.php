...
<?php try{ ?>
<body>
	<form method="post" action="envasesLote-v3.php" id="frm">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			<div class="row">
				...
				<table class="table table-bordered" id="tabla1">
					<thead>
						...
						<tr>
							<th><input type="text" name="nlote" id="nlote" class="form-control input-sm" maxlength="5" autocomplete="off" required value="<?php $nlote; ?>" /><input type="hidden" name="ilote" id="ilote" /></th>
			...
			<div class="row">
				<div class="col-md-6">
					<label><pre class="lx-pre" id="lblMsgInformacion">Seleccione el informe haciendo click en la celda deseada (fondo celeste)</pre></label></br>
				</div>
			</div>
			...
			if($tipoInf == 0){
				...
				$ilote = isset($_POST['ilote']) ? $_POST['ilote'] : false;
				//echo "<pre>";
				//echo $nlote."</br>";
				//echo $ilote."</br>";
				//echo "</pre>";
			...
			//Lotes x Fecha		
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST'){
					if($tipoenv){
						...
						$res = $DB->prLotesFechav11($fecha,$tenv);
...
<script type="text/javascript">
	...
	jq(document).ready(function(){
		...
		jq.fn.chkPattern = function(sLote1){
			sLoteInc = sLote1;
			var pattern = new RegExp('^[0-9]{4}$|^[0-9]{5}$');
			var pattern2 = /^[A-Z]{4}?[0-9]{1}$/;
			if (pattern.test(sLoteInc)) {
				jq('#nlote').css("background-color","#92FF8A");
				jq('#nlote').css("color","#A605FA");
				jq('#nlote').val(sLote);
				iLote = 'O';
				jq.fn.setHiddenV('ilote', iLote);
				bResnLote = true;
				console.log("Función chkPattern if True...");
			}else if (pattern2.test(sLoteInc)){
				if(jq('#nlote').val() !== ''){
					sLote = sLoteInc;
					jq('#nlote').css("background-color","#92FF8A");
					jq('#nlote').css("color","#A605FA");
