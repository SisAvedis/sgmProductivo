...
<?php try{ ?>
<body>
	<form method="post" action="auditoriaM-v1-2-2.php" id="frm">
    <div class="container">
		<!-- HEADER (start) -->
...
<script type="text/javascript">
	...
	jq(document).ready(function(){
		...
		jq.fn.chkPattern = function(sAuditoria){
			var pattern = new RegExp('^[0-9]{4}$|^[0-9]{6}$');
			if (pattern.test(sAuditoria)) {
				jq('#nauditoria').css("background-color","#92FF8A");
				jq('#nauditoria').css("color","#A605FA");
				bRes = true;
			}else{
				if(jq('#nauditoria').val() !== ''){
					jq('#nauditoria').css("background-color","#FAE605");
					jq('#nauditoria').css("color","#A605FA");
					jq("#nauditoria").focus();
				}
				bRes = false;
		...
		jq.fn.chkData = function(){
			...
			//console.log("iInsReg entrando a chkData -> "+iInsReg);
			for(j = 0 ;j < iInsReg ;j++){
				var lj = j.toString().length
				switch(lj) {
				//Menor 10
				case 1:
					lhi = 'LOH'+'00'+j;
					lti = 'LOT'+'00'+j;					
					lhival = jq('#'+lti).val();
					jq.fn.setHiddenV(lhi, lhival);
					if (lhival == ''){
						jq('#'+lhi).val('0');
					}
					//validación chkLote undefined
					if (typeof lhival !== 'undefined' && lhival !== false) {
						bLote = jq.fn.chkLote(lhival);
						if (bLote == true){
							jq('#'+lti).css("background-color","#FFFFFF");
							jq('#'+lti).css("color","#000000");
							aData.push(true);
						}else{
							jq('#'+lti).css("background-color","#EE2C25");
							jq('#'+lti).css("color","#FFFFFF");
							aData.push(false);
						}
					}
				break;
			...
			bootbox.confirm(
				...
				if(result){
					...
					if (sSerie !== ''){
						...
						//clientesgrupo
						var newRow = newRow+"<td><pre class='lx-pre'><input type='text' name='"+sIXL+"' class='form-control input-sm' id='"+sIXL+"' maxlength=3  autocomplete='off' value='"+ sClie +"' disabled /></pre><input type='hidden' value="+ sClid +" name='"+sIXH+"' id='"+sIXH+"' /></td>";
						//lote
						...
						//tipo de envase
						var newRow = newRow+"<td><pre class='lx-pre'><input type='text' name='"+sENV+"' class='form-control input-sm' id='"+sENV+"' maxlength=3  autocomplete='off' value="+ sEnv +" /></pre><input type='hidden' value="+ sEnv +" name='"+sENH+"' id='"+sENH+"' /></td>";
						//rampa backup
		...
		//Event binding on dynamically created elements | ComboBox Clientes Grupo Formulario Principal
		jq("#divDetalleAuditoria").on('click','#btnadd.btn.btn-success', function(){
			console.log("Entrando en bResAdd");
			bResAdd = true;
			console.log("bResAdd -> "+bResAdd);
			//jq.fn.insertaRegistroAuditoria();
		});
		
		jq.fn.chkData2 = function(){
			...
			jq('#divDetalleAuditoria #tablaUno tbody tr td pre .form-control.input-sm.remito').each(function (index) {
				...
				var sAuditoria = jq(this).prop('value');
				if (sAuditoria.length == 6){
					var bRe = jq.fn.chkPattern(sAuditoria);
					if (bRe){
						bRemitos = true;
					}else{
						bRemitos = false;
		...
		/*
		jq('#nauditoria').keypress(function(e) {
			sAuditoria = jq(this).val();
			console.log('bkeyPressed en nauditoria -> '+bkeyPressed);
			if(e.which == 13 && bkeyPressed == false) {
				jq.fn.chkPattern(sAuditoria);
				if(bRes == true){
					bkeyPressed = true;
					jq.fn.procesaAuditoria(sAuditoria);
				}
			}
		});
		*/
		...
		jq('#frm').on('submit', function(e){
			...
			sconfirmsg = "Desea modificar la auditoría N° "+ sAuditoria +"?";
			sSubmit = jq("#btnsubmit").text();
			if (returnValue == true && bResAdd == false && sSubmit == 'Actualizar'){
				bootbox.confirm({
					message: sconfirmsg,
						size: 'small',
						buttons: {
			...
			}else if(returnValue == true && bResAdd == false && sSubmit == 'Consultar'){
				jq.fn.procesaAuditoria();
			
			}else if(returnValue == false && bResAdd == false && sSubmit == 'Actualizar'){
				bootbox.alert({
					size: 'small',
					title: "<span style='color:#FF9803;font-size:1.6em;'>"+"<b>"+"Información"+"</b>"+"</span>"+"</br>",
					message: "<span style='color:#5ABCF7;'>"+"<b>"+"Datos inválidos"+"</b>"+"</span>"+"</br>",
					callback: function() {
						jq("#btnsubmit").text("Actualizar");
					}
                });
