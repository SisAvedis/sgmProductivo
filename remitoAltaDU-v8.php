...
<?php try{ ?>
<body>
	<form method="post" action="remitoAltaDU-v8.php" id="frm" >
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			<div class="row">
				<div class="col-md-2">
					...
					<label><pre class="lx-pre" id="msgTAB">Ingrese Número + TAB</pre></label></br>
				...
				<div class="col-md-2"id="lblExisteRemitoDup" >
					<label><pre class="lx-pre">Número de Remito Dup</pre></label></br>
					<label><pre class="lx-pre" id="msgTABDUP">Ingrese Número + TAB</pre></label></br>
					<pre class="lx-pre"><input type="text" name="nremitodup" id="nremitodup" class="form-control input-sm" maxlength="6" required value="<?php $nremitodup; ?>" /></pre>
				</div>
			...
			$nremitodup = isset($_POST['nremitodup']) ? $_POST['nremitodup'] : false;;
			...
			if($otipoProducto && $otipoProducto == 'LG' && $productoid && validaSerie($nremito) && validaSerie($nremitodup)){
				$envS = 0;
				$devS = 0;
				$ecapH = 0;
				$dcapH = 0;
				...
				foreach ($_POST as $clave=>$valor){
					...
					if(substr($clave,0,5) == "ECAPH" || substr($clave,0,5) == "DCAPH"){
						if(substr($clave,0,3) == "ECA" && trim($valor) <> '' && strlen($valor) > 1){
							$seriesCSV = $seriesCSV.$valor.$clave.",";
							$ecapH++;
						}
				...
				$nremitodup = $DB->sanitize($_POST['nremitodup']);
				$clienteid = explode("|",$_POST['cliente']);
				$fecha = date("Y-m-d", strtotime($_POST['fecha']));
				$motivo = isset($_POST['mot']) ? $_POST['mot'] : false;
				
				
				$res = $DB->prInsertarCSVDUv7($fecha,$nremito,$nremitodup,$clienteid[1],$productoid[0],$productoid[1],$seriesCSV,$prop,$tipoenvase,$clienteid[0],$tipoRemito,$motivo,$idusuario);
				...
				<?php
					...
					?>
				<?php
					}elseif($otipoProducto && $otipoProducto == 'GR' && $productoid && validaSerie($nremito)){
						$DB= new Database();
						$nremito = $DB->sanitize($_POST['nremito']);
						...
						$res = $DB->prInsertarGRv2($fecha,$nremito,$clienteid[1],$productoid[0],$productoid[1],$estado,$serie,$volumen,$prop,$tipoenvase,$clienteid[0],$partida,$certificado,$tdestino[0],$tipoRemito,$idusuario);
						$row_cnt = mysqli_num_rows($res);
						if ($row_cnt == 0){
							$message= "No se insertaron movimientos";
							$class="alert alert-info";
...
<script type="text/javascript">
	...
	//Chequea si ya existe el remito - al salir del input nremito - luego de seleccionar Tipo de Producto
	function chkRemitoExiste(sRemito, sTipo, nradioVal, callbackFn){
		//console.log("Valor de nradioVal -> "+nradioVal);
		if(sRemito.trim() !== ''){
			jq.ajax({
				url:"./ajaxConsultaRemito-v9.php",
				dataType: "json",
	...
	function chkRemitoExisteNIR(sRemito, callbackFn){
		jq.ajax({
			url:"./ajaxConsultaExisteRemito-v5.php",
			dataType: "json",
	...
	//Chequea si ya existe el remito - al salir del input nremito - Inicio de Alta Remito
	function chkRemitoExisteUno(sRemito, callbackFn){
		if(sRemito.trim() !== ''){
			jq.ajax({
				async: false,
				url:"./ajaxConsultaRemitoDU-v8.php",
				dataType: "json",
				data: {"nremito": sRemito},
				type:"POST",
				success:function(response){
	...
	//05-09-2022
	//Trae Motivos
	function getMotivo(callbackFn){
		//console.log('Dentro de getMotivo...');
		jq.ajax({
			url:"./ajaxGetMotivo.php",
			dataType: "json",
			type:"POST",
			success:function(response){
				callbackFn(response);
	...
	jq(document).ready(function(){
		...
		jq('#msgTABDUP').css("background-color","#B8D4FE");
		jq('#msgTABDUP').css("color","#000333");
		//var smsgTAB = "Ingrese Remito y presione TAB";
		...
		//Event binding on dynamically created elements | Opcion TipoProducto
		//LG -> Todos los productos GR -> Oxígeno
		jq("#lblExisteRemito").on('click','.oTP', function(){
			nradioVal = jq(this).val();
			
			jq.ajax({
				url:"./ajaxConsultaProducto-v1.php",
				dataType: "json",
				data: {"tipoProducto": nradioVal},
				type:"POST",
				success:function(response){
					var len = response.length;
					for(var i=0; i<len; i++){
						var header1 = response[i].header1;
						jq("#producto").html(header1);
					}
				}
			});
			...
			//09-09-22
			if(nradioVal == 'LG'){
				getMotivo(function(data) { // data = the JSON retrieved
					var len = data.length;
					var iarr = 0;
					var aarr; 
					for(var i=0; i<len; i++){
					...
					for(var i=0; i<iarr; i++){
						amotivo.push({"text":aarr[1][i],"value":aarr[0][i]});
					} 
				});
			}
			//09-09-22 - Fin
		...
		jq.fn.chkData = function(){
			...
			if(envEn == envStr && envEn == ecapStr && envStr == ecapStr && ecapStr0 == 0){
				bmotivo = false;
				aData.push(true);
			}else if(envEn == envStr && envEn == ecapStr && envStr == ecapStr && envEn == ecapStr0){
				//console.log('Pasó por True de bmotivo...'),
				bmotivo = true;
				aData.push(true);
			}else{
				aData.push(false);
			...
			if(bRemitoIngresadoDup == true){
				//console.log("Dentro de validacion por true...");
				aData.push(true);
			}else{
				//console.log("Dentro de validacion por false...");
				aData.push(false);
			}
		...
		jq.fn.cleanMsg = function(bValue){
			if(bValue == true){
				...
				jq("#lblExisteRemito3").html(sdatoremito);
				jq('#msgTAB').show();
				jq("#nremito").val("");
		...
		//Event binding on dynamically created elements | Combo Tanque
		...
		jq("#nremito").focusout('input', function(){
			...
			chkRemitoExisteUno(sRemito, function(data) {
				...
				var len = data.length;
				for(var i=0; i<len; i++){
					bRemitoIngresado = data[i].header20;
					var scolmod0 = data[i].header10;
					var scolmod1 = data[i].header11;
					var scolmod2 = data[i].header12;
					...
					if(bRemitoIngresado == true){
						...
						//31-01-2023
						jq('#nremitodup').css("background-color","#FFFFFF");
						jq('#nremitodup').css("color","#000000");
						jq("#nremitodup").val("");
						jq("#msgTABDUP").hide();
						...
						oProCmb = data[i].header6;
						//stitTre = data[i].header5;
						//sMSGDos = data[i].header6;
						stitTre = data[i].header7;
						sMSGTre = data[i].header8;
						sMSGCua = data[i].header9;
		...
		jq("#nremitodup").focusout('input', function(){
			
			sRemitoDup = jq(this).val();
			//var envOpcion = jq(".env:checked").val();
			chkRemitoExisteUno(sRemitoDup, function(data) {
				var len = data.length;
				for(var i=0; i<len; i++){
					bRemitoIngresadoDup = data[i].header20;
					if(bRemitoIngresadoDup == true){
						jq('#nremitodup').css("background-color","#92FF8A");
						jq('#nremitodup').css("color","#000000");
					}else{
						//console.log("Dentro de if por false -> "+bRemitoIngresadoDup);
						jq('#nremitodup').css("background-color","#EE2C25");
						jq('#nremitodup').css("color","#FFFFFF");
					}
				}
			});
			//console.log("Valor de bRemitoIngresadoDup dentro de focusout -> "+bRemitoIngresadoDup);
			jq.fn.chkData();
		});
		...
		jq('#frm').on('submit', function(e){	
			...
			var formData = new FormData(jq("#formulario")[0]);
				...
				case 'LG':
					...
					if(bmotivo){
						...
						callback: function (result) {
							if(result == null){
								jq.fn.setHiddenV('mot', 0)
							}else{
								jq.fn.setHiddenV('mot', result)
								var sconfirmsg = '';
								if(devBool == false){
									sconfirmsg = "Va a dar de alta el remito "+sRemito+" con entregas solamente. Es correcto?";
								}else if(envBool == false){
									sconfirmsg = "Va a dar de alta el remito "+sRemito+" con devoluciones solamente. Es correcto?";
								}else{
									sconfirmsg = "Va a dar de alta el remito "+sRemito+". Es correcto?";
								}
						...
						bootbox.confirm({
							...
							callback: function (result) {
								console.log('Valor de result desde else de bmotivo... '+result);
								if(result == true && returnValue == true){
									chkRemitoExisteNIR(sRemito, function(data) { // data = the JSON retrieved
										var len = data.length;
										for(var i=0; i<len; i++){
							...
							callback: function (result) {
								console.log('Valor de result desde else de bmotivo (envBool y devBool... '+result);
								if(result == true && returnValue == true){
									chkRemitoExisteNIR(sRemito, function(data) { // data = the JSON retrieved
									var len = data.length;
									for(var i=0; i<len; i++){
