...
<?php try{ ?>
<body>
	<form method="post" action="remitosVolumen-v4.php">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if ($clienteOption && $tipoinf){
				...
				if ($tipoinf == "R"){
					$tinf = 'Resumen';
					$res = $DB->prRemitosVolumenv4($fechaI,$fechaF,$clienteid_xl[0],1, $propiedad, $tenv);
				}elseif ($tipoinf == "D"){
					$res = $DB->prRemitosVolumenv4($fechaI,$fechaF,$clienteid_xl[0],2, $propiedad, $tenv);
					$tinf = 'Detalle';
				}
					...
					<tbody>
						<tr>
							...
							<?php }elseif ($tipoinf == "R") {?>
						        ...
						        <td class="<?php echo $claseFondo;?>"><?php if(isset($row->volumen) && !is_null($row->volumen)){
											echo number_format($row->volumen, 2, ",", ".");
											}elseif(!isset($row->volumen) && is_null($row->volumen)){
												echo "0.0";
											}else{
												echo "Error";
