...
<?php try{ ?>
<body>
	<form method="post" action="remitosVolumen-v5.php">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if ($clienteOption && $tipoinf && $idproductoOption){
				...
				if ($tipoinf == "R"){
					$tinf = 'Resumen';
					$res = $DB->prRemitosVolumenv5($fechaI,$fechaF,$clienteid_xl[0],1, $propiedad, $tenv, $producto[0]);
				}elseif ($tipoinf == "D"){
					$res = $DB->prRemitosVolumenv5($fechaI,$fechaF,$clienteid_xl[0],2, $propiedad, $tenv, $producto[0]);
					$tinf = 'Detalle';
				}
				...
				if ($row_cnt == 0){
					$message= "No se obtuvieron registros. "."(Desde: ".$fechaI." Hasta: ".$fechaF."). Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Informe: ".$tinf.". Propiedad: ".$propiedad.". Envase: ".$tenv.". Producto: (".$producto[1].") ".$producto[2];
					$class="alert alert-info";
				}elseif ($row_cnt == 1){
					$message= "Se obtuvo ".$row_cnt." registro. "."(Desde: ".$fechaI." Hasta: ".$fechaF."). Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Informe: ".$tinf.". Propiedad: ".$propiedad.". Envase: ".$tenv.". Producto: (".$producto[1].") ".$producto[2];
					$class="alert alert-success";
				}else{
					$message= "Se obtuvieron ".$row_cnt. " registros. "."(Desde: ".$fechaI." Hasta: ".$fechaF."). Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Informe: ".$tinf.". Propiedad: ".$propiedad.". Envase: ".$tenv.". Producto: (".$producto[1].") ".$producto[2];
					$class="alert alert-success";
				}
					...
					<tbody>
						<tr>
							...
							?>					
								...
								<td class="<?php echo $claseFondo;?>"><?php if(isset($row->volumen)){echo number_format($row->volumen, 2, ",", ".");}else{echo "Error";}?></td>
								...
								<td class="text-center"><?php if(isset($row->descripcion)){echo $row->descripcion;}else{echo "Error";}?></td>
							<?php }elseif ($tipoinf == "R") {?>
								<?php
									if(isset($row->ER)){
										$resultado = $row->ER;
