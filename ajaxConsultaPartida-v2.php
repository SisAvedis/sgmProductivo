...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	$DB= new Database();
	$res = $DB->prConsultaPCTv4($nradioval);
	...
	if($row_cnt > 0){
		...
		for($tre=0;$tre<$row_cnt;$tre++){
			$header1 = $header1."<tr>";
			if($tre<10){
				$header1 = $header1."<td><pre class='lx-pre'><input type='text' name='REM0".strval($tre)."' class='form-control input-sm' id='REM0".strval($tre)."' maxlength=13  autocomplete='off' value=".$t_arr[$tre][0]." disabled/></pre><input type='hidden' value=".$t_arr[$tre][0]." name='REH0".strval($tre)."' id='REH0".strval($tre)."'/></td>";
				if($t_arr[$tre][1] !== "N/D"){
				...
				$header1 = $header1."<td><pre class='lx-pre'><input type='text' name='REM".strval($tre)."' class='form-control input-sm' id='REM".strval($tre)."' maxlength=13  autocomplete='off' value=".$t_arr[$tre][0]." disabled/></pre><input type='hidden' value=".$t_arr[$tre][0]." name='REH".strval($tre)."' id='REH".strval($tre)."'/></td>";
				if($t_arr[$tre][1] !== "N/D"){
