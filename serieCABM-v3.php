...
<body>
	<form method="post" action="serieCABM-v3.php" id="frm">
    <div class="container">
		<!-- HEADER (start) -->
			...
			*/
			...
			<div class="row">
				<div class="col-md-3">
					...
					<label><pre class="lx-pre"><input type="text" name="nremito" id="nremito" class="form-control input-sm" maxlength="13" autocomplete="on" required value="<?php $nremito; ?>" /></pre></label>
					<!--<hr>-->
				</div>
				
				<div class="col-md-1" id="producto">
					<label><pre class="lx-pre">Prod</pre></label></br>
					<!--<hr>-->
				
				</div>
				<!--<div class="col-md-0" id="imagen">-->
				<!--</div>-->
			...
			<div class="row">	
				...
				<div class="col-md-1" id="lblimagen">
				</div>
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST'){
				if ($nradio == "A"){
					...
					$res = $DB->prABMSeriev3($nremito,$envdev,$nserie,$capacidad[0],$nradio,$idusuario);
					...
					if ($row_cnt == 0){
						...
						$message = $message."Producto: ".$producto."</br>";
					...
					//echo "</pre>";
					$res = $DB->prABMSeriev3($nremito,$envdev,$nserie,$capacidad[0],$nradio,$idusuario);
					//$res = $DB->prABMSeriev3($nremito,$envdev,$nserie,$scapacidad,$nradio);
					$row_cnt = mysqli_num_rows($res);
					if ($row_cnt == 0){
					...
					$res = $DB->prABMSeriev3($nremito,$envdev,$nserieTotal,$capacidad[0],$nradio,$idusuario);
					$row_cnt = mysqli_num_rows($res);
					if ($row_cnt == 0){
...
<script type="text/javascript">
	...
	jq(document).ready(function(){
		...
		//Limpia div Mensajes
		jq.fn.cleandivMSG = function(bOne,bTwo,bThr,bFou,bFiv,bSix){
			if(bOne == true){
			...
			if(bSix == true){
				jq('#lblimagen').children().each(function (index) {
					jq(this).remove();
				});
				jq('#preimagen').remove();
			}
		...
		jq.fn.setOptionCss = function(sValue,sprop,stenv,sest){
			...
			switch(sValue) {
				...
				case 0:
					...
					jq("input:radio[name=EnvDev][value=E]").removeAttr("checked");
					jq("input:radio[name=EnvDev][value=D]").removeAttr("checked");
					...
					jq("#selABMCapacidad").css("background-color",'#F5F5F5');
					break;
				case 1:
					jq('#lblPropNP').css("background-color","#F5F5F5");
					jq('#lblPropNP').css("color","#000333");
        ...
        jq.fn.achicaImg = function(sObj,sTipo){
			...
			switch(sTipo){
				//CIL
				case 'CIL':
					var currentImgWidth = parseFloat(currentImgWidth)*0.17;  
					var currentImgHeigth = parseFloat(currentImgHeigth)*0.08;
				break;
				//TER
				case 'TER':
					var currentImgWidth = parseFloat(currentImgWidth)*0.20;  
					var currentImgHeigth = parseFloat(currentImgHeigth)*0.08;
				break;
				//GRANEL
				case 'ZZZ':
					var currentImgWidth = parseFloat(currentImgWidth)*0.10;  
					var currentImgHeigth = parseFloat(currentImgHeigth)*0.10;
				break;
		...
		jq.fn.inputradio = function(nradioVal){ 
			...
			//jq.fn.cleandivMSG(true,true,false,false,false,true);
			...
			if (nradioVal == 'A') {
					...
					jq.fn.cleandivMSG(true,true,true,true,true,true);
					...
					*/
					jq("#nremito").val('');
					jq("#nserie").val('');
					jq('#nremito').css("background-color","#FFFFFF");
			}
			else if (nradioVal == 'B') {
					jq('#lblMsgInformacion').css("background-color","#D1E3FD");
					jq('#lblMsgInformacion').css("color","#000333");
			...
			jq.ajax({
				...
				success:function(response){
					...
					for(var i=0; i<len; i++){
						...
						var slblcod = response[i].var21;
						var scodigo = response[i].var22;
						var oimagen = response[i].var23;
						...
						if(ifecha == 1 && iid_xl == 1 && iprop == 1 && itipo == 1 && bres == true){
							jq("#producto").append(oimagen);
							jq.fn.achicaImg("imagenP",stenv);
							jq("#lblimagen").html(slblcod);
							jq('#lblproducto').css("background-color","#92FF8A");
							jq('#lblproducto').css("color","#140CC5");
							//jq.fn.achicaFont("producto");
							//jq("#producto").text(sMsgUno);
							//jq("#cmbCapacidad").prop("disabled",false);
							
							jq("#nserieModi").prop("disabled", false);
						...
						//Remito Granel	
							...
							jq("#nserieModi").prop("disabled",true);
							jq("#nserieModi").val("");
							jq("input:radio[name=EnvDev]").attr("disabled",true);
							jq("input:radio[name=prop]").attr("disabled",true);
							jq("input:radio[name=env]").attr("disabled",true);
							...
							//jq("#cmbCapacidad").prop("disabled",true);
					...
					for(var i=0; i<len; i++){
						...
						if(imov > 0){
							...
							if(stenv !== 'ZZZ'){
								...
								var sMsg = "<label><pre class='lx-pre' id='lbldivSerie'>N° Envase: N/D</pre></label>";
								jq("#divSerie").html(sMsg);
								jq('#lbldivSerie').css("background-color","#FAE605");
								jq('#lbldivSerie').css("color","#140CC5");
							}
						...
						if(icap > 0){
							...
							var sMsgUno = "<label><pre class='lx-pre' id='lblMSGSerie'>No hay registros del envase "+sSerie+"</pre></label>";
							jq("#divMensaje").html(sMsgUno);
							jq('#lblMSGSerie').css("background-color","#FAE605");
							jq('#lblMSGSerie').css("color","#140CC5");
							//jq.fn.setOptionCss(8);
						}
					}
					console.log("Valor de bSerieVal -> "+bSerieVal);
					jq.fn.chkData();
				}
			})
		}
		...
		jq.fn.serieModificar = function(oSerie){
			...
			jq.fn.cleandivMSG(false,false,false,false,false,false);
			var lblcapacidadExiste = "<label><pre class='lx-pre' id='capacidadExiste'></pre></label>";
			if(oSerie == 'ns'){
				var sSerie = jq("#nserie").val();
				var odiv = 'divSerie';
				...
				success:function(response){
					...
					for(var i=0; i<len; i++){
						...
						if(imov > 0){
							...
							/*
							if(stenv !== 'ZZZ' && oSerie == 'ns'){
								var sMsg = "<label><pre class='lx-pre' id='lbldivSerie'>No Encontrado</pre></label>";
								jq("#divSerie").html(sMsg);
								jq('#lbldivSerie').css("background-color","#92FF8A");
								jq('#lbldivSerie').css("color","#140CC5");
								var sMsgUno = "<label><pre class='lx-pre' id='lblMSGSerie'>El Envase "+sSerie+" no se encuentra en el remito "+sRemito+"  </pre></label>";
								jq("#divMensaje").html(sMsgUno);
								jq('#lblMSGSerie').css("background-color","#FAE605");
								jq('#lblMSGSerie').css("color","#140CC5");
							}else if(stenv !== 'ZZZ' && oSerie == 'nsmodi'){
								var sMsg = "<label><pre class='lx-pre' id='lbldivSerie'>No Encontrado</pre></label>";
								jq("#divSerieModi").html(sMsg);
								jq('#lbldivSerie').css("background-color","#92FF8A");
								jq('#lbldivSerie').css("color","#140CC5");
							}
							*/
		...
		jq.fn.serieEliminar = function(nradioVal){
			...
			jq.fn.cleandivMSG(false,true,false,false,false,false);
			var lblcapacidadExiste = "<label><pre class='lx-pre' id='capacidadExiste'></pre></label>";
			var sSerie = jq("#nserie").val();
			var sRemito = jq("#nremito").val();
			var aCapacidad = [];
		...
		jq.fn.chkData = function(){
			var aData = [];
			console.log("bCapVal -> "+bCapVal+" |-| bEnvDevVal -> "+bEnvDevVal+" |-| bSerieVal -> "+bSerieVal+" |-| nradioVal -> "+nradioVal);
