...
<?php try{ ?>
<body>
	<form method="post" action="remitoBM-v3.php" id="frm">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			<div class="row">
				...
				<div class="col-md-1" id="lblExisteRemito41">
				</div>
				<div>
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			if($tipo <> "Granel"){
				...
				if ($bmOpcion == "B"){
					...
					$res = $DB->prBMRemitoLGv3($fecha,$fecha,$nremito,$nremito,$nid_xl,$nid_xl,$pr,$pr,$tipo,$bmOpcion,$idusuario);
					...
					$res = $DB->prBMRemitoLGv3($fecha,$fechaM,$nremito,$nremitomodi,$nid_xl,$clienteid[0],$pr,$prM,$tipo,$bmOpcion,$idusuario);
					$row_cnt = mysqli_num_rows($res);
				}
				?>
			...
			<?php		
				...
				if ($bmOpcion == "B"){
					...
					$res = $DB->prBMRemitoGRv3($fecha,$fecha,$nremito,$nremito,$nid_xl,$nid_xl,$vopacetaid[0],$vopacetaid[0],
					...
					$res = $DB->prBMRemitoGRv3($fecha,$fechaM,$nremito,$nremitomodi,$nid_xl,$clienteid[0],$vopacetaid[0],$vol,
...
<script type="text/javascript">
	...
	jq(document).ready(function(){
		...
		jq.fn.chkPattern = function(sRemito1){
			...
			var pattern = /^([0-9]{4})+([1-9]{1})+([0-9]{8})$/;
			if (pattern.test(sRemito1)) {
				console.log("Por True -> "+sRemito1);
				jq('#nremitomodi').css("background-color","#92FF8A");
				jq('#nremitomodi').css("color","#A605FA");
				return true;
			}else{
				if(jq('#nremitomodi').val() !== ''){
					console.log("Por False -> "+sRemito1);
		...
		jq.fn.cleandivEDI = function(){
			...
			jq('#lblExisteRemito41').children().each(function (index) {
				jq(this).remove();
			});
		...
		jq.fn.procesaRemito = function(sRemito, nradioVal){
			...
			//console.log("Valor de len -> "+len);
				for(var i=0; i<len; i++){
					...
					jq("#lblExisteRemito3").addClass(scolmod0);
					jq("#lblExisteRemito4").removeClass();
					jq("#lblExisteRemito4").addClass(scolmod2);
					jq("#lblExisteRemito41").removeClass();
					jq("#lblExisteRemito41").addClass(scolmod3);
					...
					if(bRemitoExiste == true){
						...
						var sMSGCioc = data[i].header58;
						var sMSGCinu = data[i].header59;
						...
						var sLblCioc = "<label id='cioc'><pre class='lx-pre'></pre></label></br>";
						var sLblCinu = "<label id='cinu'><pre class='lx-pre'></pre></label></br>";
						...
						jq("#lblExisteRemito3").append(sLblCioc+sLblCinu);
						jq("#cioc pre").attr('id', 'msgCioc');
						jq("#cinu pre").attr('id', 'msgCinu');
						jq("#lblExisteRemito4").append(sLblCin);
						jq("#cin pre").attr('id', 'msgCIN');
						jq("#lblExisteRemito4").append(sLblSei);
						jq("#sei pre").attr('id', 'msgSEI');
						jq("#lblExisteRemito41").append(sLblSie);
						jq("#sie pre").attr('id', 'msgSIE');
						jq("#lblExisteRemito41").append(sLblOch);
						...
						jq('#msgCioc').css("background-color","#F5F5F5");
						jq('#msgCioc').css("color","#000333");
						jq("#msgCioc").text(sMSGCioc);
						jq('#msgCinu').css("background-color",sBGColor);
						jq('#msgCinu').css("color","#000333");
						jq("#msgCinu").text(sMSGCinu);
		...
		jq("#nremito").focusout('input', function(){
			//console.log("FocusOut de nremito...");
		...
		jq.fn.chkData = function(){
			...
			switch(nradioVal) {
				case 'M':
					...
					if(bRemito == true && sRemito !== sRemitoModi){
						aData.push(false);
					}else{
						aData.push(true);
					}
