...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if($nremito && $bremito) {
		...
		$res = $DB->prRemitoDetallev8($nremito);
		$row_cnt = mysqli_num_rows($res);
		if($row_cnt == 0){
			
			
			$header51 = "Producto";
			$header52 = "Seleccione producto";
			//Todos los productos
			$tLP = 0;
			$DB1= new Database();
			...
			foreach($mov_arr as $sel){
				$header53 = $header53.$sel;
			}
			$header53 = $header53."</select></pre></label>";
			...
			$select_arr[0] = "<label><pre class='lx-pre'>Cliente</pre></label>"."</br>"."<label><pre class='lx-pre'><select name='cliente' id='cliente' class='form-control input-sm'>"."<option value='0'>Clientes</option>";
				...
				$header3 = "<label><pre class='lx-pre'>Tipo</pre></label>"."</br>"."<label><pre class='lx-pre'><input type='radio' name='env' value='CIL' class='envRadio' required checked/>CIL<input type='radio' name='env' value='TER' class='envRadio' required/>TER</pre></label><input type='hidden' name='envH' id='envH'/>";
				...
				$headerB = "Si/No";
				$headerC = "No";
				$headerD = "col-md-3";	//Producto (Etiqueta + Combo)
				$headerE = "col-md-1";	//Etiqueta Listo (Si/No)
												
			$return_arr[] = array("header1" => $header1, "header2" => $header2, "header3" => $header3, "header4" => $header4, "header5" => $header5, "header6" => $header6, "header51" => $header51, "header52" => $header52, "header53" => $header53, "header7" => $header7, "header8" => $header8, "header9" => $header9, "header10" => $header10, "header11" => $header11, "header12" => $header12, "header13" => $header13, "headerA" => $headerA, "headerB" => $headerB, "headerC" => $headerC, "headerD" => $headerD, "headerE" => $headerE);		
			...
			echo json_encode($return_arr);
				...
				$return_arr[] = array("header1" => $header1, "header2" => $header2, "header7" => $header7, "header8" => $header8, "header13" => $header13);
				echo json_encode($return_arr);
			}
