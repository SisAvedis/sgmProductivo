...
<?php try{ ?>
<body>
	<form method="post" action="remitoAltaSI-v7.php" id="frm" >
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if($propiedad && $tipoenv && validaSerie($nremito)){
				...
				$tipoRemito = 'RSUBI';
				$res = $DB->prInsertarCSVSIv6($fecha,$nremito,$clienteid[1],$seriesCSV,$prop,$tipoenvase,$clienteid[0],$tipoRemito,$idusuario);
...
<script type="text/javascript">
	...
	function chkRemitoExiste(sRemito, sTipo, callbackFn){
		...
		if(sRemito.trim() !== ''){
			jq.ajax({
				url:"./ajaxConsultaRemitoSI-v7.php",
				dataType: "json",
	...
	jq(document).ready(function(){
		...
		*/
		...
		jq("#nremito").focusout('input', function(){
			...
			/*
			chkRemitoExiste(sRemito, function(data) {
				jq('#lblRemito').children().each(function (index) {
				jq(this).remove();
				});
				jq('#lblRemito2').children().each(function (index) {
