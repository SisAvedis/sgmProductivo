...
<body>
	<form method="post" action="cHits-v5.php" id="frm">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if(validaNumero($clienteOption && $cantidad)){
				...
				$DB= new Database();
				$res = $DB->prRepeticionesSeriev4($fechaI,$fechaF,$clienteid_xl[0],$tenv,$cantidad,$producto[0]);
				$row_cnt = mysqli_num_rows($res);
				$fechaI = date_format(date_create($fechaI),"d-m-Y");
				$fechaF = date_format(date_create($fechaF),"d-m-Y");
				if ($row_cnt == 0){
					$message= "No se obtuvieron registros. (Desde: ".$fechaI." Hasta: ".$fechaF."). Cantidad de Repeticiones: ".$cantidad.". Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Producto: (".$producto[1].") ".$producto[2];
					$class="alert alert-info";
				}elseif ($row_cnt == 1){
					$message= "Se obtuvo ".$row_cnt." registro. (Desde: ".$fechaI." Hasta: ".$fechaF."). Cantidad de Repeticiones: ".$cantidad.". Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Producto: (".$producto[1].") ".$producto[2];
					$class="alert alert-success";
				}else{
					$message= "Se obtuvieron ".$row_cnt." registros. (Desde: ".$fechaI." Hasta: ".$fechaF."). Cantidad de Repeticiones: ".$cantidad.".  Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Producto: (".$producto[1].") ".$producto[2];
					$class="alert alert-success";
				}
