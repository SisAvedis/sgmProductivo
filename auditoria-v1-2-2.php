...
<?php try{ ?>
<body>
	<form method="post" action="auditoria-v1-2-2.php" id="frm">
    <div class="container">
		<!-- HEADER (start) -->
	...
	<?php
			if ($_SERVER['REQUEST_METHOD'] == 'POST'){
				...
				$res = $DB->prInsertarAUCSVv1($fecha,$seriesCSV,$clienteid[0],$idusuario);
...
<script type="text/javascript">
	...
	function getEnvaseCliente(sDate, sCliente, callbackFn){
		jq.ajax({
			url:"./ajaxEnvaseAuditoria-v1-2-2.php",
			dataType: "json",
	...
	jq(document).ready(function(){
		...
		//Limpia la grilla de auditorias
		...
		jq("#divDetalleAuditoria").on('click','.btn', function(){
			...
			// attribute exists?
			if (typeof attr !== 'undefined' && attr !== false && attr !== 'btnadd') {
		...
		jq.fn.actualizacantidadregistros = function(iCantidadRegistros){
			jq("#lbcantidadE").html("<pre class='lx-pre'>Total de Registros: "+iTotalReg+" | Registros Insertados Manualmente: "+iCantidadRegistros+"</pre>");
		}
		...
		//Event binding on dynamically created elements | TextBox Lote
		...
		//Event binding on dynamically created elements | TextBox Lote
		jq("#divDetalleAuditoria").on('change','.form-control.input-sm.lote', function(){
			var lti = jq(this).attr('id');
			var ltival = jq(this).val();
			bLote = jq.fn.chkLote(ltival);
			if (bLote == true){
				jq('#'+lti).css("background-color","#FFFFFF");
				jq('#'+lti).css("color","#000000");
			}else{
		...
		jq('#frm').on('submit', function(e){
			...
			sconfirmsg = "Confirma el alta de la auditoría?";
