...
<?php try{ ?>
<body>
	<form method="post" action="UltimoEnviado-v4-1.php">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			*/
			...
			<div class="row">
				...
				<table class="table table-bordered" id="tabla1">
					<thead>
						...
						<tr>
							<!--<th>Ultimo_E</th><th>Ultimo_D</th><th>Ultimo_DA</th><th>Cantidad</th><th>Envases</th><th>NP </th><th>SP</th><th>CIL</th><th>TER</th>-->
						...
						<tr>
							...
							<th class="text-center"><input type="radio" name="propiedad" value="NS" required /></th></th>
			...
			if ($clienteOption && $detallepor && $idproductoOption){
				...
				$res = $DB->prinUltimoEnviadov411($fecha,$clientesCSV,$detallepor,$propiedad,$tenv,$estado,$producto[0]);
				...
				?>
					<tbody>
						<tr>
							...
							?>
								<td class="text-left <?php echo $claseULinea;?>">
								<?php 
								if(isset($row->id_xl) && isset($row->nombre) && $row->nombre <> ''){
									echo '('.$row->id_xl.') '.$row->nombre;
									}elseif(isset($row->id_xl) && isset($row->nombre) && $row->nombre == ''){
										echo '';
										}else{
											echo "Error";
										}
	...
	$(document).ready(function(){
		...
		$('#cliente').multiselect({
			...
			buttonText: function(options, select) {
				//console.log('Valor de options.length (UEv4-1) -> '+options.length);
                if (options.length === 0) {
                    return 'Cliente';
                }
