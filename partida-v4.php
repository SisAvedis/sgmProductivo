...
<?php try{ ?>
<body>
	<form method="post" action="partida-v4.php" id="frm">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			<div class="row">
				...
				<table class="table table-bordered" id="tabla">
					<thead>
						<tr>
							<th class="text-center" colspan=5>Consultar</th>
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST'){
				if($nradio){
					...
					if($nradio == "K"){
						...
						$res = $DB->prActualizaPCTv2($seriesCSV);
		   ...
		   <?php
				...
				<?php		
						...
						$res = $DB->prConsultaPCTv3($fechaI,$fechaF,$nradio);
					//}
					switch ($nradio) {
					case "C":
						$sMsg = "Consultar - Completos";
...
<script type="text/javascript">
	...
	jq(document).ready(function(){
		...
		jq("#datepickerI").datepicker({
				dateFormat: "dd-mm-yy"
		});
		jq("#datepickerF").datepicker({
				dateFormat: "dd-mm-yy"
		});
		...
		jq.fn.inputradio = function(nradioVal){
			jq.ajax({
				url:"./ajaxConsultaPartida-v2.php",
				dataType: "json",
		...
		jq("#lblPartida1").on('focusout','.form-control', function(){
			...
			var iPaCe = jq(this).attr('id').substring(3,4);
			...
			//console.log("Dentro de FocusOut, valor de iTest -> "+iPaCe+" |-| "+iParCer);
			switch(sParCer) {
				//Partida
				case "P":
					...
					//console.log("Dentro de FocusOut, valor de sParCerVal -> "+iParCer+" |-| "+sParCerVal);
		...
		jq("#lblPartida1").on('click','.chkBox', function(){
			...
			//console.log("aValoresFull[3][4] click chkBox -> "+aValoresFull[3][iMovId]+" |-| "+aValoresFull[4][iMovId]);
			//console.log("aValoresFullI[3][4] click chkBox -> "+aValoresFullI[3][iMovId]+" |-| "+aValoresFullI[4][iMovId]);
			//console.log("jq(this).prop('checked') -> "+(jq(this).prop('checked')+" |-| "+iMovId)+" |-| "+sChkId+" |-| "+iPaCe);
			jq.fn.chkDataPCT();
		});
		...
		jq("input[name=abmOpcion]").click(function () {
			...
			if(jq("input:radio[name=abmOpcion][value='K']").prop('checked') == true){
				...
				jq('#datepickerI').prop('required',false);
				jq('#datepickerI').prop('disabled', true);
				jq('#datepickerI').val('');
				jq('#datepickerF').prop('required',false);
				jq('#datepickerF').prop('disabled', true);
				jq('#datepickerF').val('');
			}else{
				jq('#datepickerI').prop('required',true);
				jq('#datepickerI').prop('disabled', false);
				jq('#datepickerF').prop('required',true);
				jq('#datepickerF').prop('disabled', false);
		...
		jq.fn.chkDataPCT = function(){
			...
			for(k = 0 ;k < 5 ;k++){
				for(j = 0 ;j < ilen ;j++){
					//console.log("["+k+"]["+j+"] -> "+aValoresFull[k][j]+" ["+k+"]["+j+"] -> "+aValoresFullI[k][j]);
					//console.log("["+k+"]["+j+"] -> "+aDataFull[k][j]);
				}
			}
			*/
