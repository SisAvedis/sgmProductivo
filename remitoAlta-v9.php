...
<?php try{ ?>
<body>
	<form method="post" action="remitoAlta-v9.php" id="frm" >
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			<div class="row">
				<div class="col-md-3">
					...
					<label><pre class="lx-pre" id="msgTAB">P. Venta + Remito (sin ceros) + TAB</pre></label></br>
			...
			if($otipoProducto && $otipoProducto == 'LG' && $productoid && validaSerie($nremito)){
				...
				$res = $DB->prInsertarCSVv8($fecha,$nremito,$clienteid[1],$productoid[0],$productoid[1],$seriesCSV,$prop,$tipoenvase,$clienteid[0],$tipoRemito,$motivo,$idusuario);
				...
				<?php
						...
						$res = $DB->prInsertarGRv3($fecha,$nremito,$clienteid[1],$productoid[0],$productoid[1],$estado,$serie,$volumen,$prop,$tipoenvase,$clienteid[0],$partida,$certificado,$tdestino[0],$tipoRemito,$idusuario);
						$row_cnt = mysqli_num_rows($res);
						if ($row_cnt == 0){
							$message= "No se insertaron movimientos";
							$class="alert alert-info";
...
<script type="text/javascript">
	...
	function chkRemitoExiste(sRemito, sTipo, nradioVal, callbackFn){
		if(sRemito.trim() !== ''){
			jq.ajax({
				url:"./ajaxConsultaRemito-v11.php",
				dataType: "json",
	...
	function chkRemitoExisteNIR(sRemito, callbackFn){
		jq.ajax({
			url:"./ajaxConsultaExisteRemito-v6.php",
			dataType: "json",
	...
	function chkRemitoExisteUno(sRemito, callbackFn){
		if(sRemito.trim() !== ''){
			jq.ajax({
				url:"./ajaxConsultaRemito-v10.php",
				dataType: "json",
				data: {"nremito": sRemito},
				type:"POST",
				success:function(response){
