...
<?php try{ ?>
<body>
	<form method="post" action="remitosGranelVolumen-v3.php">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if ($clienteOption && $tipoinf && $idproductoOption){
				...
				if ($tipoinf == "R"){
					$tinf = 'Resumen';
					$res = $DB->prRemitosGranelv3($fechaI,$fechaF,$clienteid_xl[0],1,"ZZZ", $producto[0]);
				}elseif ($tipoinf == "D"){
					$res = $DB->prRemitosGranelv3($fechaI,$fechaF,$clienteid_xl[0],2,"ZZZ", $producto[0]);
					$tinf = 'Detalle';
				}
				...
				if ($row_cnt == 0){
					...
					$message= "No se obtuvieron registros. (Desde: ".$fechaI." Hasta: ".$fechaF."). Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Informe: ".$tinf.". Producto: (".$producto[1].") ".$producto[2];
				}elseif ($row_cnt == 1){
					$class="alert alert-success";
					$message= "Se obtuvo 1 registro.  (Desde: ".$fechaI." Hasta: ".$fechaF."). Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Informe: ".$tinf.". Producto: (".$producto[1].") ".$producto[2];
				}else{
					$class="alert alert-success";
					$message= "Se obtuvieron ".$row_cnt." registros. (Desde: ".$fechaI." Hasta: ".$fechaF."). Cliente: (".$clienteid_xl[0].") ".$clienteid_xl[1].". Informe: ".$tinf.". Producto: (".$producto[1].") ".$producto[2];
				}
					...
					<tbody>
						...
						<tr class="<?php echo $claseFondo;?>">
							<?php if ($tipoinf == "D"){ ?>
								...
								<td class="text-right"><?php if(isset($row->volumen)){echo number_format($row->volumen, 2, ",", ".");}else{echo "Error";}?></td>
							<?php }elseif ($tipoinf == "R") {?>
								<td><?php if(isset($row->id_xl) && isset($row->nombre)){echo '('.$row->id_xl.') '.$row->nombre;}else{echo "Error";}?></td>
								<td class="text-center"><?php if(isset($row->producto)){echo $row->producto;}else{echo "Error";}?></td>
						        <td class="text-right"><?php if(isset($row->volumen)){echo number_format($row->volumen, 2, ",", ".");}else{echo "Error";}?></td>
