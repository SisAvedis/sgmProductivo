...
<?php try{ ?>
<body>
	<form method="post" action="remitosVolumen-v1.php">
    <div class="container">
		<!-- HEADER (start) -->
        ...
        <div class="table-wrapper">
			...
			if ($clienteOption && $tipoinf){
				...
				if ($tipoinf == "R"){
					$tinf = 'Resumen';
					$res = $DB->prRemitosVolumenv1($fechaI,$fechaF,$clienteid_xl[0],1, $propiedad, $tenv);
				}elseif ($tipoinf == "D"){
					$res = $DB->prRemitosVolumenv1($fechaI,$fechaF,$clienteid_xl[0],2, $propiedad, $tenv);
					$tinf = 'Detalle';
				}
					...
					<tbody>
						<tr>
							...
							?>					
								...
								<td><?php if(isset($row->ER)){echo $row->ER;}else{echo "Error";}?></td>
							<?php }elseif ($tipoinf == "R") {?>
								<?php
									if(isset($row->ER)){
										$resultado = $row->ER;
